...
function mortgageCalculator(principal, interestRate, years, creditScore){
    if (creditScore > 740) {
        return (`${name}, your monthly rate is $${(monthlyRate *.95).toFixed(2)}`)
        }else if(creditScore < 660){
            return (`${name}, your monthly rate is $${(monthlyRate *1.05).toFixed(2)}`)
        }else{
            return (`${name}, your monthly rate is $${monthlyRate}`);
        }    
     }
     mortgageCalculator(200000, .05, 30, 750)
...
let starting= interestRate * .98;
for(let i = 0, i < 10, i++){
    starting = starting * 1.005;
